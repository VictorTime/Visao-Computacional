...
def parse_args():
    ...
    # ADDED OR MODIFIED: argumento para retomar de um checkpoint
...
def train(config, train_loader, model, criterion, optimizer):
    ...
    for input, target, _ in train_loader:
        ...
        if config['deep_supervision']:
            ...
            iou_, dice_, recall_, specificity_, precision_ = indicators(outputs[-1], target)
            ...
            iou_, dice_, recall_, specificity_, precision_ = indicators(output, target)
...
def main():
    ...
    if dataset_name == 'busi':
        ...
        mask_ext = '.png'
    elif dataset_name == 'mass_separada':
        mask_ext = '.png'
    else:
        mask_ext = '.png'

    # Caminhos para as pastas de treino e validação
    train_dir = os.path.join(config['data_dir'], config['dataset'], 'train')
    val_dir = os.path.join(config['data_dir'], config['dataset'],'val')

    # Coletando os identificadores das imagens de treino
    train_img_ids = sorted(glob(os.path.join(train_dir, 'images','*' + img_ext)))
    train_img_ids = [os.path.splitext(os.path.basename(p))[0] for p in train_img_ids]

    # Coletando os identificadores das imagens de validação
    val_img_ids = sorted(glob(os.path.join(val_dir, 'images','*' + img_ext)))
    val_img_ids = [os.path.splitext(os.path.basename(p))[0] for p in val_img_ids]

    # Agora, train_img_ids e val_img_ids possuem as imagens dos respectivos diretórios.
    print(f"Número de imagens de treino: {len(train_img_ids)}")
    print(f"Número de imagens de validação: {len(val_img_ids)}")
    # Data loading code
    ...
    start_epoch = 0
     ...
     # ADDED OR MODIFIED: Se houver um checkpoint para retomar
    if config['resume'] is not None:
        ...
        # Carrega o scheduler, se disponível
        if 'scheduler' in checkpoint and scheduler is not None:
            scheduler.load_state_dict(checkpoint['scheduler'])
        # Carrega o melhor valor de IoU, epoch, etc.
    ...
    for epoch in range(config['epochs']):
        ...
        if val_log['iou'] > best_iou:
            ...
            }, f'{output_dir}/{exp_name}/model.pth')
